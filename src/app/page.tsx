...
const SUITE_CARDS = [
	...
	},
	{
		title: "Nova",
		description:
			"Same brain as Sherpa. New embodiment. Launching in stealth.",
		cta: "Get notified →",
		featured: true,
	},
];
				...
				return (
					<div className="space-y-2">
						{[
							...
							<div key={item} className="flex items-center gap-2 text-[10px] text-[#65697a]">
								<span className={`inline-flex h-3 w-3 items-center justify-center rounded-[3px] ${i < 4 ? "bg-[#5b5ce8] text-white" : "border border-[#d5d7df] text-[#c4c7d2]"}`}>
									{i < 4 ? "✓" : "×"}
								</span>
								<span>{item}</span>
							</div>
				...
				return (
					<div className="space-y-2">
						{[
							...
							<div key={label} className={`flex items-center justify-between rounded-full px-3 py-1.5 text-[10px] ${i === 1 ? "bg-[#5b5ce8] text-white" : "bg-white text-[#6f7485]"}`}>
								<span>{label}</span>
				...
				return (
					<div className="flex h-full items-end gap-3 px-5 pb-2 pt-3">
						{[34, 40, 52, 64, 72, 56].map((h, i) => (
							<div key={i} className={`w-6 rounded-t-md ${i === 4 ? "bg-[#5b5ce8]" : "bg-[#c9cbf4]"}`} style={{ height: h }} />
						))}
					</div>
				);
			case "pin":
				return (
					<div className="relative flex h-full w-full items-center justify-center overflow-hidden">
						...
						<div className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center">
							<svg viewBox="0 0 24 24" className="h-16 w-16 text-[#5b5ce8]" fill="currentColor" aria-hidden="true">
				...
				return (
					<div className="flex h-full items-end gap-2 px-4 pb-2 pt-4">
						{[24, 36, 32, 48, 56, 72, 64].map((h, i) => (
							<div key={i} className={`w-5 rounded-t-md ${i === 5 ? "bg-[#5b5ce8]" : "bg-[#cfd0f6]"}`} style={{ height: h }} />
						))}
					</div>
				);
			case "list":
				return (
					<div className="space-y-2">
						{[
							...
							<div key={item} className="flex items-center gap-2 text-[10px] text-[#6b7182]">
								<span className={`h-3 w-3 rounded-[2px] ${i < 4 ? "bg-[#5b5ce8]" : "bg-[#e6e8ef]"}`} />
								<span>{item}</span>
							</div>
						))}
					</div>
			...
			case "network":
				return (
					<div className="relative h-full">
						<svg viewBox="0 0 200 110" className="h-full w-full px-4 py-4" fill="none" aria-hidden="true">
							<path d="M28 58 C55 30, 72 82, 98 58 S145 30, 172 58" stroke="#d8dcf7" strokeDasharray="4 4" strokeWidth="1.5" />
							{[28, 60, 92, 124, 156, 178].map((x, i) => (
								<g key={x}>
									<circle cx={x} cy={i % 2 === 0 ? 54 : 58} r="5" fill={i === 2 ? "#5b5ce8" : "#d2d6f7"} />
								</g>
							))}
						</svg>
					</div>
				...
				return (
					<div className="space-y-3">
						{[
							...
							<div key={item} className="flex items-center gap-2 text-[10px] text-[#6b7182]">
								<span className={`h-2.5 w-2.5 rounded-full ${i === 0 ? "bg-[#5b5ce8]" : i === 1 ? "bg-[#9a9df1]" : "bg-[#d6d8e7]"}`} />
								<span>{item}</span>
							</div>
						))}
					</div>
			...
			>
				<div className="relative mx-auto max-w-[1182px] text-center">
					...
					<p className="mt-5 text-sm sm:text-base text-[#7a7a7a] max-w-[820px] mx-auto leading-relaxed">
						...
						campaign and a hundred back-office agents from a single
						proprietary stack. Real-time tokens flow into an in-house SLM
						that learns from every wedge.
					</p>
				...
				<div className="mx-auto max-w-[1182px] grid gap-10 lg:grid-cols-[1.1fr_1fr] lg:gap-16">
					<div>
						...
						<p className="mt-4 text-sm sm:text-base text-[#7a7a7a] max-w-[520px] leading-relaxed">
							Most vendors sell you one of these. Darwix is the only
							place all three are built in-house, and each layer feeds
							the next on every conversation.
						</p>
					</div>
			...
			<section className="bg-white px-4 sm:px-6 lg:px-[129px] pb-20 sm:pb-24 lg:pb-28">
				<div className="mx-auto max-w-[1182px]">
					<div className="text-center">
						...
						<h2 className="mt-4 text-3xl sm:text-4xl lg:text-[46px] font-semibold text-[#4b4b4b] leading-tight">
							One meeting. <span className="text-[#5b5ce8]">Multiple actions</span>
							{" "}
						...
						<div className="flex h-full flex-col rounded-[18px] border border-[#e4e4e8] bg-[#F8F8F8] p-5 sm:p-6 shadow-[0_6px_18px_rgba(46,55,88,0.08)]">
							...
							<div className="mt-8">
								{SLM_METRICS.map((metric) => (
									<div key={metric.label} className="border-b border-dashed border-[#d8d9df] py-3 first:pt-0">
					...
					<div className="mt-10">
						<p className="mb-4 text-center align-middle text-[19px] font-bold leading-[100%] tracking-[-0.19px] text-[#4b4b4b]">
							Trusted by Top Enterprises Globally
						</p>
						<LogoMarquee
							logos={TRUST_LOGOS}
							barClassName="bg-[#5b5ce8]"
						/>
					</div>
					...
					<div className="mt-20 sm:mt-24">
						<div className="text-center max-w-[760px] mx-auto">
							...
							<h2 className="mt-4 text-[28px] sm:text-[36px] lg:text-[42px] font-semibold text-[#4b4b4b] leading-[1.15] tracking-[-0.02em]">
								...
								<br />
								<span className="text-[#5b5ce8]">the previous one’s experience.</span>
							</h2>
							...
							{WEDGE_CARDS.map((card) => (
								...
								>
									...
									<div className="mt-7 border-t border-dashed border-[#e6e8ef] pt-4 flex items-end justify-between gap-4">
										...
										<div className={`text-right text-[12px] font-semibold leading-none ${card.metricTone}`}>
											{card.metricValue}
										</div>
									</div>
								</div>
				...
				<div className="mt-20 sm:mt-24">
					<div className="text-center max-w-[760px] mx-auto">
						...
						<h2 className="mt-4 text-[32px] sm:text-[40px] lg:text-[42px] font-semibold text-[#4b4b4b] leading-[1.15] tracking-[-0.02em]">
							Built for one industry.
							<br />
							<span className="text-[#5b5ce8]">Tuned to four sub-verticals.</span>
						</h2>
					</div>

					<div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4 items-stretch">
						{SCENARIO_CARDS.map((card) => (
							<div key={card.title} className="flex h-full min-h-[430px] flex-col overflow-hidden rounded-[20px] border border-[#eceef4] bg-white shadow-[0_8px_22px_rgba(37,44,97,0.05)]">
								<div className="flex h-[210px] items-center border-b border-[#f0f1f6] p-4 sm:p-5">
									{renderScenarioVisual(card.visual)}
								</div>
								<div className="flex flex-1 flex-col p-4 sm:p-5">
									<h3 className="text-[18px] font-semibold tracking-[-0.01em] text-[#4b4b4b] leading-[1.2]">
										{card.title}
									</h3>
									<p className="mt-3 text-[12px] leading-[1.55] text-[#7a7a7a]">
										{card.description}
									</p>
									<div className="mt-auto pt-5 border-t border-dashed border-[#e7e8f0] flex items-center justify-between gap-3">
										<div className="text-[10px] text-[#9ca0ad]">
											{card.metricLeft}
										</div>
										<div className="text-[10px] font-semibold text-[#5b5ce8]">
											{card.metricRight}
										</div>
									</div>
								</div>
						...
						<div className="grid items-center gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-12">
							<div className="max-w-[520px]">
								<h2 className="text-[32px] sm:text-[42px] lg:text-[52px] font-semibold leading-[1.03] tracking-[-0.03em] text-[#4b4b4b]">
									This is what buys
									<br />
									you <span className="text-[#5b5ce8]">10× productivity</span> and <span className="text-[#5b5ce8]">5× more value</span> on field.
								</h2>
				...
				<div className="mt-20 sm:mt-24">
					<div className="text-center">
						...
						<h2 className="mt-4 text-[32px] sm:text-[40px] lg:text-[44px] font-semibold text-[#4b4b4b] leading-[1.1] tracking-[-0.03em]">
							<span className="text-[#5b5ce8]">Four</span> agents. One brain.
						</h2>
					</div>
						...
						{SUITE_CARDS.map((card) => (
							...
							>
								...
								<div className="mt-10 pt-5">
									<a href="#" className="text-[12px] font-semibold text-[#5b5ce8]">
										{card.cta}
									</a>
								</div>
							</div>
						...
						<div className="grid items-stretch gap-5 lg:grid-cols-[1fr_36px_1fr_36px_1fr_36px_1fr]">
							<div className="flex min-h-[338px] flex-col rounded-[14px] border border-[#eceef6] bg-white p-5 text-left shadow-[0_10px_24px_rgba(37,44,97,0.04)]">
								<div className="flex items-start justify-between">
									<span className="text-[11px] font-semibold text-[#5b5ce8]">01</span>
							...
							<div className="flex min-h-[338px] flex-col rounded-[14px] border border-[#eceef6] bg-white p-5 text-left shadow-[0_10px_24px_rgba(37,44,97,0.04)]">
								<div className="flex items-start justify-between">
									<span className="text-[11px] font-semibold text-[#5b5ce8]">02</span>
									<span className="rounded-full bg-[#f0efff] px-3 py-1 text-[11px] font-semibold text-[#5b5ce8]">
										Support
									</span>
								</div>
								...
								<div className="mt-4 flex items-center gap-3 rounded-[9px] border border-[#eff0f5] bg-[#fbfbfd] px-3 py-3">
									...
									<div className="flex flex-1 items-center gap-1">
										{[18, 26, 19, 30, 24, 34, 23, 31, 20, 28, 18].map((height, index) => (
											<span
												key={index}
							...
							<div className="flex min-h-[338px] flex-col rounded-[14px] border border-[#eceef6] bg-white p-5 text-left shadow-[0_10px_24px_rgba(37,44,97,0.04)]">
								<div className="flex items-start justify-between">
									<span className="text-[11px] font-semibold text-[#5b5ce8]">03</span>
									<span className="rounded-full bg-[#f0efff] px-3 py-1 text-[11px] font-semibold text-[#5b5ce8]">
										Physical
									</span>
								</div>
								...
								<div className="mt-4 rounded-[9px] border border-[#eff0f5] bg-[#fbfbfd] px-3 py-3 text-[10px] leading-relaxed text-[#858995]">
									<div className="grid grid-cols-[1fr_auto] gap-3">
										...
										<span className="font-semibold text-[#5b5ce8]">Sherpa visit</span>
										<span>Pre-approval:</span>
										<span className="font-semibold text-[#5b5ce8]">EMI shift</span>
										<span>Stage:</span>
										<span className="font-semibold text-[#5b5ce8]">04 - 05</span>
									</div>
								</div>
								<p className="mt-4 text-[13px] leading-[1.45] text-[#767a85]">
									Branch-officer-side assist with everything Sherpa captured plus
									everything voice confirmed. Real-time fulfilment, structured
									capture.
								</p>
								<div className="mt-auto border-t border-dashed border-[#e6e8ef] pt-4">
									<div className="flex items-center justify-between gap-3 text-[9px] text-[#8f94a3]">
							...
							<div className="flex min-h-[338px] flex-col rounded-[14px] border border-[#ccd1ff] bg-[#f0f1ff] p-5 text-left shadow-[0_10px_24px_rgba(91,92,232,0.06)]">
								<div className="flex items-start justify-between">
									<span className="text-[11px] font-semibold text-[#5b5ce8]">04</span>
