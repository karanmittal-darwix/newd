...
export default function SherpaPage() {
	...
	const ActiveCapabilityCard = capabilityCards[activeIndex] || AgentAssistNudges;
	return (
													...
													{WAVE_BARS.map((height, barIndex) => {
														...
														return (
															<rect
																...
																rx={3}
																className="wave-rect"
																style={{ animationDelay: `${(index * 0.12 + (barIndex % 6) * 0.06).toFixed(2)}s` }}
															/>
														);
													})}
												</svg>
												...
												>
													<div className="flex items-start justify-between gap-4">
														<div className="flex items-start gap-4">
															...
															<div>
																<p className={`text-sm sm:text-base font-semibold ${isActive ? "text-indigo-600" : "text-gray-700"}`}>
																	{item.title} {" "}
